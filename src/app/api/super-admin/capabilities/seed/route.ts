  ...
  createCapability({
    ...
    navigation: [
      createNavigationEntry({
        ...
        href: '/iso-design',
        icon: 'Layers',
        feature: 'iso-design-development',
        roles: ['admin', 'gerente', 'super_admin'],
      }),
    ],
...
export const POST = withAuth(async () => {
  try {
    await Promise.all(
      ISO9001_CAPABILITIES.map(capability =>
    ...
    return NextResponse.json({
      success: true,
      data: {
        seeded: ISO9001_CAPABILITIES.length,
        capabilities: ISO9001_CAPABILITIES.map(capability => ({
          id: capability.id,
