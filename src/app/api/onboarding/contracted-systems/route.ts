...
export const GET = withAuth(
  async (request: NextRequest, _context, auth) => {
    try {
      ...
      // Federated onboarding for users without organization yet:
      // expose active platform systems as selectable options.
      if (!requestedOrgId) {
        const platformSnapshot = await db.collection('platform_systems').get();
        const systems: ContractedSystem[] = platformSnapshot.docs
          .map(doc => {
            const data = doc.data() as Record<string, unknown>;
            const status =
              typeof data.status === 'string' ? String(data.status) : 'active';
            return {
              systemId: doc.id,
              systemName: String(data.name || doc.id),
              status,
              modulesEnabled: Array.isArray(data.modules)
                ? (data.modules as string[])
                : [],
            };
          })
          .filter(system => (system.status || 'active') !== 'inactive');
