...
import { Phone, PhoneOff, Radio, Terminal } from 'lucide-react';
...
export function VoiceAuditor() {
  ...
  const handleFunctionCall = async (name: string, args: any) => {
    ...
    try {
      if (name === 'get_my_audits') {
        ...
        if (audits.length === 0) {
          return { message: 'No tienes auditorías en curso actualmente.' };
        }
        ...
        return {
          audits: audits.map(a => ({
            ...
            scope: a.scope,
            status: a.status,
            normPoints: a.selectedNormPoints, // Array de strings
          })),
        };
      }

      if (name === 'create_finding') {
        // Inferencia: Usar la primera auditoría en curso si no se especifica
        const { audits } = await AuditService.list(usuario.organization_id, {
          status: 'in_progress',
        });
        ...
        if (!activeAudit) {
          return {
            error:
              'No hay ninguna auditoría en curso para registrar el hallazgo. Inicia una primero.',
          };
        }
        ...
        return {
          success: true,
          findingId,
          message: `Hallazgo registrado correctamente en auditoría ${activeAudit.auditNumber}`,
        };
      }
    } catch (e: any) {
  ...
  // Animación del visualizador (simulada)
