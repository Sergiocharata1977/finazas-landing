...
export async function sendWhatsAppMessage(
  ...
  const formattedFrom = formatWhatsAppNumber(TWILIO_WHATSAPP_NUMBER);

  const url = `${getTwilioBaseUrl()}/Messages.json`;
  ...
  try {
    ...
    return {
      success: false,
      error: error.message || 'Error de conexión con Twilio',
    };
  }
}
...
export async function getMessageStatus(messageSid: string): Promise<{
  ...
  const url = `${getTwilioBaseUrl()}/Messages/${messageSid}.json`;
